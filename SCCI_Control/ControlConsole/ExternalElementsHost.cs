...
namespace PE.ControlConsole
{
    ...
    internal class ExternalElementsHost: IDisposable
    {
        ...
        private void Plot(IEnumerable<object> YValues, double Step, double InitX)
        {
            try
            {
                PlotForm.RunPlot(YValues.Select(Convert.ToDouble), Step, InitX);
            }
            catch (Exception e)
            {
                Console.WriteLine(e.Message);
        ...
        private void Plot(IEnumerable<object> YValues1, IEnumerable<object> YValues2, double Step, double InitX)
        {
            try
            {
                PlotForm.RunPlot(YValues1.Select(Convert.ToDouble).ToList(), YValues2.Select(Convert.ToDouble).ToList(), Step, InitX);
            }
            catch (Exception e)
            {
                Console.WriteLine(e.Message);
        ...
        private void PlotScatter(IEnumerable<object> XValues, IEnumerable<object> YValues)
        {
            try
            {
                PlotForm.RunPlot(XValues.Select(Convert.ToDouble).ToList(), YValues.Select(Convert.ToDouble).ToList(), true);
            }
            catch (Exception e)
            {
                Console.WriteLine(e.Message);
