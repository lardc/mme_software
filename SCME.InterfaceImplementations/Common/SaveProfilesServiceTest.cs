...
{
    public class SaveProfilesServiceTest<TDbCommand, TDbConnection> : ISaveProfileServiceTest where TDbCommand : DbCommand where TDbConnection : DbConnection
    {
        private TDbConnection _Connection;

        protected virtual string _ProfileInsertString => "INSERT INTO[dbo].[PROFILES] (PROF_NAME, PROF_GUID, PROF_TS, PROF_VERS) OUTPUT INSERTED.PROF_ID VALUES(@PROF_NAME, @PROF_GUID, @PROF_TS, @VERSION)";

        protected virtual string _ProfileConditionInsertString => "INSERT INTO [dbo].[PROF_COND](PROF_TESTTYPE_ID, PROF_ID, COND_ID, VALUE) VALUES(@PROF_TESTTYPE_ID, @PROF_ID, @COND_ID, @VALUE)";
        protected virtual string _ProfileParameterInsertString => "INSERT INTO [dbo].[PROF_PARAM](PROF_TESTTYPE_ID, PROF_ID, PARAM_ID, MIN_VAL, MAX_VAL) VALUES(@PROF_TESTTYPE_ID, @PROF_ID, @PARAM_ID, @MIN_VAL, @MAX_VAL)";
        protected virtual string _ProfileTestTypeInsertString => "INSERT INTO [dbo].[PROF_TEST_TYPE] (PROF_ID, TEST_TYPE_ID, [ORD]) OUTPUT INSERTED.PTT_ID VALUES (@PROF_ID, @TEST_TYPE_ID, @ORD)";

        protected virtual string _MMECodeToProfileInsertString => "INSERT INTO [dbo].[MME_CODES_TO_PROFILES] (MME_CODE_ID, PROFILE_ID) VALUES (" +
            "(SELECT MME_CODE_ID FROM MME_CODES WHERE MME_CODE = @MME_CODE), @PROFILE_ID)";
        protected virtual string _MMECodeToProfileDeleteString => "DELETE FROM MME_CODES_TO_PROFILES WHERE PROFILE_ID = @PROFILE_ID";
        ...
        private void LoadDictionary()
        {
            using (var reader = _LoadTestTypes.ExecuteReader())
                foreach (DbDataRecord i in reader)
                    _TestTypeIdByName.Add(i.GetString(1), i.GetInt32(0));

            using (var reader = _LoadConditions.ExecuteReader())
                foreach (DbDataRecord i in reader)
                    _ConditionIdByName.Add(i.GetString(1), i.GetInt32(0));

            using (var reader = _LoadParameters.ExecuteReader())
                foreach (DbDataRecord i in reader)
                    _ParameterIdByName.Add(i.GetString(1), i.GetInt32(0));
        }
        ...
        private int InsertProfile(MyProfile profile, string MMECode)
        {
            ...
            int id = Convert.ToInt32(_ProfileInsert.ExecuteScalar());
        ...
        private int SaveProfile(MyProfile profile, string MMECode)
        {
            ...
            InserterBaseTestParametersAndNormatives inserter = new InserterBaseTestParametersAndNormatives(profile.Id, _DbTransaction,
        ...
        public int InsertUpdateProfile(MyProfile oldProfile, MyProfile newProfile, string MMECode)
        {
            try
            {
                ...
                int id = SaveProfile(newProfile, MMECode);
                _DbTransaction.Commit();
                return id;
            }
