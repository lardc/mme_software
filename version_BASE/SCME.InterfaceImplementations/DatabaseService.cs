            ...
            _mConnection =
                new SQLiteConnection(
                    String.Format("data source={0};{1}",
                                  databasePath, dbSettings), false);
            _profilesService = new SQLiteProfilesService(String.Format("data source={0};{1}",
                                  databasePath, dbSettings));
        }
            ...
            if (_mConnection.State == ConnectionState.Open)
            {
                ...
                try
                {
                    using (var deleteCmd = _mConnection.CreateCommand())
                    {
                        ...
                        {
                            deleteCmd.CommandText = String.Format("DELETE FROM {0}", table);
