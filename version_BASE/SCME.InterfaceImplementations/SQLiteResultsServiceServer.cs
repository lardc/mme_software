    ...
    public class SQLiteResultsServiceServer : IResultsService
    {
        ...
        public void WriteResults(ResultItem result, IEnumerable<string> errors)
        {
            if (_connection != null && _connection.State == ConnectionState.Open)
            {
                ...
                try
                {
                    var devId = InsertDevice(result, GetOrMakeGroupId(result.Party), result.ProfileKey);
        ...
        private long GetOrMakeGroupId(string party)
        {
            long groupId;
            ...
            groupSelectCommand.Parameters["@GROUP_NAME"].Value = party;
            ...
            if (possibleGroupId == null)
            {
                ...
                groupInsertCommand.Parameters["@GROUP_NAME"].Value = party;
        ...
        protected virtual long InsertDevice(ResultItem result, long groupId, Guid profileId)
        {

            var deviceSelectCmd = new SQLiteCommand("SELECT D.DEV_ID FROM DEVICES D WHERE D.CODE = @CODE AND D.GROUP_ID = @GROUP_ID AND D.POS = @POS", _connection);
            ...
            deviceSelectCmd.Parameters["@CODE"].Value = result.Code;
            ...
            devInsertCommand.Parameters["@CODE"].Value = result.Code;
            devInsertCommand.Parameters["@SIL_N_1"].Value = result.StructureOrd;
            devInsertCommand.Parameters["@SIL_N_2"].Value = result.StructureName;
        ...
        private void InsertParameterValues(ResultItem result, long devId)
        {
            if (result.IsHeightMeasureEnabled)
                InsertParameterValue(devId, "Height", result.IsHeightOk ? 1 : 0, 0);
