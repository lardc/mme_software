...
namespace SCME.Service.IO
{
    internal class IOStLs
    {
        ...
        internal ushort ReadRegister(ushort Address, bool SkipJournal = false)
        {
            ...
            if(!SkipJournal)
                SystemHost.Journal.AppendLog(ComplexParts.SL, LogMessageType.Note,
                                             string.Format("SL @ReadRegister, address {0}, value {1}", Address, value));
        ...
        internal short ReadRegisterS(ushort Address, bool SkipJournal = false)
        {
            ...
            if (!SkipJournal)
                SystemHost.Journal.AppendLog(ComplexParts.SL, LogMessageType.Note,
                                             string.Format("SL @ReadRegisterS, address {0}, value {1}", Address, value));
        ...
        internal void WriteRegister(ushort Address, ushort Value, bool SkipJournal = false)
        {
            if (!SkipJournal)
                SystemHost.Journal.AppendLog(ComplexParts.SL, LogMessageType.Note,
                                         string.Format("SL @WriteRegister, address {0}, value {1}", Address, Value));
        ...
        internal void WriteRegisterS(ushort Address, short Value, bool SkipJournal = false)
        {
            if (!SkipJournal)
                SystemHost.Journal.AppendLog(ComplexParts.SL, LogMessageType.Note,
                                         string.Format("SL @WriteRegisterS, address {0}, value {1}", Address, Value));
        ...
        {
            SystemHost.Journal.AppendLog(ComplexParts.SL, LogMessageType.Note,
                                        string.Format("SL @Call, action {0}", Action));
        ...
        {
            SystemHost.Journal.AppendLog(ComplexParts.SL, LogMessageType.Note,
                                         string.Format("SL @ReadArrayFast, endpoint {0}", Address));
        ...
        {
            SystemHost.Journal.AppendLog(ComplexParts.SL, LogMessageType.Note,
                                         string.Format("SL @ReadArrayFastS, endpoint {0}", Address));
        ...
        {
            SystemHost.Journal.AppendLog(ComplexParts.SL, LogMessageType.Note,
                                         "SL @WriteCalibrationParams begin");
        ...
        {
            SystemHost.Journal.AppendLog(ComplexParts.SL, LogMessageType.Note,
                                         "SL @ReadCalibrationParams begin");
        ...
        private void MeasurementLogicRoutine(Types.Commutation.TestParameters Commutation)
        {
            try
            {
                ...
                if (!m_Parameter.IsSelfTest)
                    if (m_IOCommutation.Switch(Types.Commutation.CommutationMode.SL, Commutation.CommutationType, Commutation.Position) ==
                        DeviceState.Fault)
                    {
                        m_State = DeviceState.Fault;
                        FireVTMEvent(m_State, m_Result, 0);
                            ...
                            case Types.SL.VTMTestType.Ramp:
                                {
                                    ...
                                    WriteRegister(REG_HEATING_ENABLE,
                                                        (ushort) (m_Parameter.IsRampOpeningEnabled ? 1 : 0));
