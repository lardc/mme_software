    ...
    public class ControlLogic
    {
        ...
        private void ProcessCommunicationException(CommunicationException Ex)
        {
            ...
            try
            {
                ...
                {
                    if (!m_ControlClient.IsInitialized())
        ...
        public bool Start(Types.Gate.TestParameters ParametersGate, Types.SL.TestParameters ParametersVtm,
                          Types.BVT.TestParameters ParametersBvt, Types.Commutation.TestParameters ParametersCommutation, Types.Clamping.TestParameters ParametersClamping, bool SkipSC = false)
        {
            if (!IsServerConnected)
                return false;
            ...
            try
            {
                ...
                if (!ParametersGate.IsEnabled && !ParametersVtm.IsEnabled && !ParametersBvt.IsEnabled)
                {
                ...
                return m_ControlClient.Start(ParametersGate, ParametersVtm, ParametersBvt, ParametersCommutation, ParametersClamping);
        ...
        public bool Start(TestParameters paramsComm, Types.Clamping.TestParameters paramsClamp, List<BaseTestParametersAndNormatives> parameters, bool SkipSC = false)
        {
            ...
            try
            {
                ...
                return m_ControlClient.StartDynamic(paramsComm, paramsClamp, parameters.OfType<Types.Gate.TestParameters>().Where(t => t.IsEnabled).ToArray(), parameters.OfType<Types.SL.TestParameters>().Where(t => t.IsEnabled).ToArray(), parameters.OfType<Types.BVT.TestParameters>().Where(t => t.IsEnabled).ToArray(), parameters.OfType<Types.dVdt.TestParameters>().Where(t => t.IsEnabled).ToArray(), parameters.OfType<SctuTestParameters>().ToArray());
        ...
        public void StopMeasuringTemp()
        {
            if (!IsServerConnected) return;
            try
            {
                m_ControlClient.StopMeasureTemperature();
            }
            catch (CommunicationException ex)
            {
                ProcessCommunicationException(ex);
